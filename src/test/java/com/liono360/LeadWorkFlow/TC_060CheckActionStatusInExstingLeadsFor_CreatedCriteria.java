...
public class TC_060CheckActionStatusInExstingLeadsFor_CreatedCriteria extends Baseclass {

	@Test
	public void TC_060CheckActionStatusInExstingLeadsFor_CreatedCriteriaTest() throws InterruptedException, IOException {
...
//CREATE WORK FLOW ----------------------------------------------------------------------------------------------	    
		...
		workflow.setName().sendKeys("WorkFlow"+randomnumber()+"onLeadNumberRule");
...
// VERIFY THAT WORKFLOW CREATED AND WORKING FINE OR NOT---------------------------------------------------
	    ...
	    commonMethods.GotoExistingLead(existingLeads);
	    ...
	    boolean CheckActionStatusInExistingLead=commonMethods.CheckActionStatusInCreatedLead();
	    ...
	    if(WorkflowNameToCreate.equals(WorkflowNameAfterCreate)) {
	    	...
	    	if(CheckActionStatusInExistingLead) {
	    		  logger.info("workflow is created but actions are also created for existing  lead that is WRONG");
	    	}
	    	else {logger.info("workflow is created  and  actions are not created  for existing lead that is RIGHT");}
...
// ASSERTION--------------------------------------------------------------------------------------------------------
	   ...
	   if (WorkflowNameToCreate.equals(WorkflowNameAfterCreate)&&ActionStatusInHistory&&ActionStatusInCreatedLead&&(!CheckActionStatusInExistingLead)) {
