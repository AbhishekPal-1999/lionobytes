...
public class TC_019CreateAccountTerritoryWithMultipleRules extends Baseclass {

	@Test
	public void TC_019CreateAccountTerritoryWithMultipleRule() throws IOException, InterruptedException {
		...
		for (int i = 1; i <= 2; i++) {
			...
			if (i == 9) {
				driver.findElement(By.xpath("(//input[@type=\"text\"])[14]")).sendKeys("277124");
			}
...
// Assertion
		...
		for (int i = 1; i <= 2; i++) {
			String territoryNameAfterCreate = driver.findElement(By.xpath("(//tr)[" + (3 + i) + "]//td[1]//span"))
					.getText();
