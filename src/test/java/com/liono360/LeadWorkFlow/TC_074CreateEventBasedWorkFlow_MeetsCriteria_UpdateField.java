...
public class TC_074CreateEventBasedWorkFlow_MeetsCriteria_UpdateField extends Baseclass{

	@Test
	public void TC074_CreateEventBasedWorkFlow_MeetsCriteria_UpdateField() throws InterruptedException, IOException {
...
//CREATE WORK FLOW ----------------------------------------------------------------------------------------------	    
	 ...
	 //driver.findElement(By.xpath("//span[text()='Satyendra Bhardwaj']")).click();
		...
		logger.info("created And EveryTime Edited till meets criteria clicked");
...
// ADD MANUAL CONDITOON HERE----------------------------------------------------------------------
	    ...
	    workflow.setConditionvalue().sendKeys("LionoLead"+randomnumber());
...
// CREATE A LEAD ACCORDING TO THIS RULE-----------------------------------------------------------
	    ...
	    driver.switchTo().window(tabs.get(1));
	    String CreatedLead=commonMethods.CreateLeadToupdateField(ruleValue);
	    driver.switchTo().window(tabs.get(0));
	    Thread.sleep(6000);
	    workflow.clickPreviewdata().click();
	    logger.info("preview button clicked");
...
// VERIFY THAT WORKFLOW CREATED AND WORKING FINE OR NOT---------------------------------------------------
	    ...
	    commonMethods.EditExistingLeadToUpdateFields(existingLeads);

	    driver.switchTo().window(tabs.get(2));
	    Thread.sleep(1000);
	    String NewlyCreatedLead=commonMethods.CreateLeadToupdateField(ruleValue);
	    driver.switchTo().window(tabs.get(0));
	    Thread.sleep(1000);
	    ...
	    driver.switchTo().window(tabs.get(1));
	    Thread.sleep(1000);
	    boolean FieldUpdateStatusInNewLead=commonMethods.checkFieldUpdateStatus(NewlyCreatedLead,list1);
	    driver.switchTo().window(tabs.get(2));
	    Thread.sleep(1000);
	    boolean FieldUpdateStatusInExistedLead=commonMethods.checkFieldUpdateStatus(existingLeads,list1);
	    ...
	    if(WorkflowNameToCreate.equals(WorkflowNameAfterCreate)) {
	    	...
	    	if(FieldUpdateStatusInNewLead) {
	    		  logger.info("workflow is created and Fields are also updated in newly created lead ");
	    	}
	    	else {logger.info("workflow is created  but fields are not updated  in newly created lead");}
	    	
	    	if(FieldUpdateStatusInExistedLead) {
	    		  logger.info("workflow is created and fields are also updated  in existing  lead ");
	    	}
	    	else {logger.info("workflow is created  but fields are not updated  in existing  lead");}
...
// ASSERTION--------------------------------------------------------------------------------------------------------
	    ...
	    if (WorkflowNameToCreate.equals(WorkflowNameAfterCreate)&&ActionStatusInHistory&&FieldUpdateStatusInExistedLead&&FieldUpdateStatusInNewLead) {
