...
public class TC_017CreatedWorkflowTimeBasedSs extends Baseclass {
	String wfname = null;

	@Test
	public void CreatedWorkflowTimeBasedSs() throws InterruptedException {
		redirectToCRM();
	...
	//	logger.info("select default channel");
		...
		Thread.sleep(2000);
		logger.info("click add rule");
		driver.findElement(By.xpath("//div[normalize-space()='TimeBasedSavedAutomation']")).click();
		Thread.sleep(2000);
		...
		workflow.AccountName().sendKeys("AccountAutomation"+randomnumber());
		logger.info("sendkeys on accountname");
		Thread.sleep(4000);
      String account=workflow.AccountName().getAttribute("value");
