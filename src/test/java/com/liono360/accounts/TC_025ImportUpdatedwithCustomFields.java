...
public class TC_025ImportUpdatedwithCustomFields extends Baseclass {
	
	@Test
	public void ImportUpdatedwithCustomFieldsTest () throws InterruptedException, AWTException {
		driver.get(baseURL);
		...
		Thread.sleep(5000);
		//explWaitToClick(account.ClickBBox());
		account.ClickBBox().click();
		logger.info("Clicked on ClickBBox button");
		...
		Thread.sleep(5000);
		account.LoadConfig().click();
		logger.info("Clicked on LoadConfig button");
		
		Thread.sleep(5000);
		account.CClickCheckbox().click();
		logger.info("Clicked on CClickCheckbox button");
